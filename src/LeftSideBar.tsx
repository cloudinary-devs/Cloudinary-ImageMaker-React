      ...
      <div className="flex-1 overflow-y-auto">
        ...
        <div className="grid grid-cols-2 gap-3">
          {templates.map((template, index) => {
            const tempImg = cld.image(`templates/${template}`);
            tempImg.resize(fill().height(320));
            const tempImgUrl = cld.image(`templates/${template}`).toURL();
            return (
            <div
              key={index}
            ...
            >
              <img src={tempImg.toURL()} alt={`Template ${index + 1}`} className="object-cover hover:opacity-80" />
            </div>
          )})}
        </div>
      </div>
